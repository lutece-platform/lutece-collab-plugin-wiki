...
public class WikiParser
{
    ...
    private boolean parseBlock(  )
    {
        ...
        { // lists
            ...
            // count list level
            for ( lc = 0;
                    ( lc <= listLevel ) && ( ( pos + lc ) < wikiLength ) && ( wikiChars[pos + lc] == listLevels[lc] );
                    lc++ )
                ;
        ...
        { // nowiki-block?
            ...
            else if ( ( ( pos + 1 ) < wikiLength ) && ( wikiChars[pos + 1] == '|' ) )
            { // mediawiki-table?

                int pp;

                for ( pp = pos + 2; ( pp < wikiLength ) && ( ( wikiChars[pp] == ' ' ) || ( wikiChars[pp] == '\t' ) );
                        pp++ )
                    ; // skip spaces
    ...
    private int parseListItem( int start )
    {
        ...
        if ( ( ( listLevels[listLevel] == '>' ) || ( listLevels[listLevel] == ':' ) ) &&
                ( wikiText.substring( start, end ).trim(  ).length(  ) == 0 ) )
...
nextChar: 
            while ( true )
            {
                ...
                if ( c == '\n' )
                {
                    ...
                    { // check for ---- <hr>
                        ...
                        for ( pp = p + 4; ( pp < end ) && ( ( wikiChars[pp] == ' ' ) || ( wikiChars[pp] == '\t' ) );
                                pp++ )
                            ; // skip spaces
                    ...
                    { // mediawiki-table?
                        ...
                        if ( ( ( p + 1 ) < end ) && ( wikiChars[p + 1] == '|' ) )
                        {
                            ...
                            for ( pp = p + 2;
                                    ( pp < end ) && ( ( wikiChars[pp] == ' ' ) || ( wikiChars[pp] == '\t' ) ); pp++ )
                                ; // skip spaces
    ...
    protected void appendImage( String text )
    {
        ...
        if ( ( uri != null ) && uri.isAbsolute(  ) && !uri.isOpaque(  ) )
        {
            String alt = escapeHTML( unescapeHTML( ( ( link.length >= 2 ) && !isEmpty( link[1].trim(  ) ) ) ? link[1]
                                                                                                            : link[0] ) );
    ...
    protected String generateTOCAnchorId( int hLevel, String text )
    {
        ...
        String id = ( ( HEADING_ID_PREFIX != null ) ? HEADING_ID_PREFIX : ( "H" + hLevel + "_" ) ) +
            translit( text.replaceAll( "<.+?>", "" ) ).trim(  ).replaceAll( "\\s+", "_" )
