 ...
 */
@RequestScoped
@Named( "wiki.xpage.wikisuggestion" )
...
public class WikiSuggestionXPage extends AbstractWikiXPage
{
    ...
    public XPage propose( HttpServletRequest request ) throws UserNotSignedException, SiteMessageException
    {
        ...
        _models.put( MARK_CURRENT_REVISION, currentRevision );
        _models.put( SecurityTokenService.MARK_TOKEN, SecurityTokenService.getInstance( ).getToken( request, ACTION_PROPOSE ) );
        populateCommonModel( _models, user );

        XPage page = getXPage( TEMPLATE_PROPOSE, locale );
    ...
    public XPage list( HttpServletRequest request ) throws UserNotSignedException
    {
        ...
        XPage page = getXPage( TEMPLATE_LIST, locale );
    ...
    public XPage viewSuggestion( HttpServletRequest request ) throws UserNotSignedException, SiteMessageException
    {
        ...
        _models.put( MARK_ITEM, item );
        _models.put( MARK_CURRENT_REVISION, currentRevision );
        _models.put( MARK_DIFF_BASE_REVISION, diffBaseRevision );
        _models.put( MARK_SUGGESTION, suggestion );
        _models.put( MARK_CAN_REVIEW, canReview );
        _models.put( MARK_IS_OWNER, isOwner );
        _models.put( MARK_MERGE_BLOCKS, mergeBlocks );
        _models.put( MARK_MERGE_HAS_CONFLICT, hasConflict );
        _models.put( SecurityTokenService.MARK_TOKEN, SecurityTokenService.getInstance( ).getToken( request, ACTION_APPROVE ) );
        _models.put( MARK_TOKEN_DELETE, SecurityTokenService.getInstance( ).getToken( request, ACTION_DELETE ) );
        populateCommonModel( _models, user );

        XPage page = getXPage( TEMPLATE_DETAIL, locale );
